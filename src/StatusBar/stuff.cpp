...
#include "StatusBar/lib.hpp"
#include "StatusBar/stuff.hpp"

#include "PlaySimulation/inter.hpp"
...
static void ST_ProcessInput(INPUTS input)
{
...

    {
        ...
        else if (cht_CheckCheat(&cheat_noclip, key) || cht_CheckCheat(&cheat_commercial_noclip, key))
        {
        ...
        else if (cht_CheckCheat(&cheat_mypos, key))
        {
            ...
            sprintf(buf, "ang=0x%x;x,y=(0x%x,0x%x)", gamePlayer.mo->angle, gamePlayer.mo->x, gamePlayer.mo->y);
            plyr->message = buf;
        }
