...
void V_Init(void)
{
    int i;
    byte *base;
    const int screen_size = SCREENWIDTH * SCREENHEIGHT;

    // stick these in low dos memory on PCs

    base = I_AllocLow(screen_size * 4);
    if (!base)
        I_Error("V_Init: failed to allocate software framebuffers");

    for (i = 0; i < 4; i++)
        screens[i] = base + i * screen_size;
}
