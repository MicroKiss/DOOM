...
default_t defaults[] =
    {
        DEFAULT_NUMBER("mouse_sensitivity", &mouseSensitivity, 5),
...
void M_LoadDefaults(void)
{
    ...
    {
        if (defaults[i].number_location)
            *defaults[i].number_location = defaults[i].number_default;
        else
            *defaults[i].string_location = (char *)defaults[i].string_default;
    }
        ...
        while (!feof(f))
        {
            ...
            if (fscanf(f, "%79s %[^\n]\n", def, strparm) == 2)
            {
                ...
                for (i = 0; i < numdefaults; i++)
                    ...
                    {
                        if (!isstring && defaults[i].number_location)
                            *defaults[i].number_location = parm;
                        else if (isstring && defaults[i].string_location)
